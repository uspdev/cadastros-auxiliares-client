...
class CursosGraduacaoClient implements CursosGraduacaoClientInterface
{
    ...
    {
        $endpoint = trim((string) $this->config->get('cadastros-auxiliares-client.graduacao_cursos.endpoint_url', ''));
        ...
        return $baseUrl . '/api/graduacao/cursos';
    }

    private function httpRequest(): PendingRequest
    {
        $timeout = max(1, (int) $this->config->get('cadastros-auxiliares-client.graduacao_cursos.timeout', 5));
        $retryTimes = max(0, (int) $this->config->get('cadastros-auxiliares-client.graduacao_cursos.retry_times', 1));
        $retrySleepMs = max(0, (int) $this->config->get('cadastros-auxiliares-client.graduacao_cursos.retry_sleep_ms', 150));
        ...
        $password = trim((string) $this->config->get(
            'cadastros-auxiliares-client.graduacao_cursos.password',
            $this->config->get('cadastros-auxiliares-client.programas.password', '')
        ));
